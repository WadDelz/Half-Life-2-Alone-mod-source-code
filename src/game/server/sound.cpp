...
//-------------------------- ALONE MOD SONG TRANSITION THROUGH LEVELS SYSTEM --------------------------//



//plays a song
void Amod_PlaySong(const char* filename, float flVolume, int iPitch, float flDuration, bool docheck = false)
{
	//build the args
	char volume[16];
	char duration[32];
	...
	Q_snprintf(volume, sizeof(volume), "%f", flVolume);
	Q_snprintf(pitch, sizeof(pitch), "%d", iPitch);
	Q_snprintf(duration, sizeof(duration), "%f", flDuration);

	//search for the command
	static ConCommand* cmd = cvar->FindCommand("_amod_playsound");
	if (!cmd)
		return;

	CCommand args;
	args.m_ppArgv[0] = "_amod_playsound";
	args.m_ppArgv[1] = filename;
	args.m_ppArgv[2] = volume;
	args.m_ppArgv[3] = pitch;
	args.m_ppArgv[4] = duration;
	args.m_ppArgv[5] = docheck ? "1" : "0";
	args.m_nArgc = 6;
...
//song transition system song item
struct CSongTransition
{
	char SongName[1028];			//name of the song
	float Volume = 1.0f;			//volume of the song
	int pitch = 100;				//pitch of the song
	float SongStartTime = 0.0f;		//the time (gpglobals->curtime) the song started
	float TimeElapsed = 0.0f;		//how long the song has been going for
	float SongDuration = 0.0f;		//the duration for the entire song
	bool bInUse = false;
};

//only 1 song can transition through a level at a time. There is a bug where
//if you have 1 song playing, transition through a level, Then in the level you
//transitioned to another song starts playing, The game's audio system can bug out,
//cause MAD frame loss, and just not allow any other audio to play. To fix this 
//i am allowing only 1 song to play at a time
static CSongTransition g_SongToTransition;

bool bShouldPlayOnceLoaded = false;

//saves/loads the current song state into the save file. Called from gameinterface.cpp
void Amod_SaveSong(ISave* save)
{
	//see if we are in use
	save->WriteBool(&g_SongToTransition.bInUse);
	if (!g_SongToTransition.bInUse)
		return;

	//fixup elapsed
	float elapsed = g_SongToTransition.TimeElapsed - (gpGlobals->curtime - g_SongToTransition.SongStartTime);

	//write the song info
	save->WriteString(g_SongToTransition.SongName);
	save->WriteInt(&g_SongToTransition.pitch);
	save->WriteFloat(&g_SongToTransition.Volume);
	save->WriteFloat(&g_SongToTransition.SongDuration);
	save->WriteFloat(&gpGlobals->curtime);
	save->WriteFloat(&elapsed);
}

void Amod_LoadSong(IRestore* restore)
{
	//see if we are in use
	restore->ReadBool(&g_SongToTransition.bInUse);
	if (!g_SongToTransition.bInUse)
		return;

	//if gpGlobals->eLoadType is not MapLoad_LoadGame then read temp values and just return.
	if (gpGlobals->eLoadType != MapLoadType_t::MapLoad_LoadGame)
	{
		char songname[32];
		int pitch;
		float volume, duration, starttime, timeelapsed;

		restore->ReadString(songname, sizeof(songname), 0);
		restore->ReadInt(&pitch);
		restore->ReadFloat(&volume);
		restore->ReadFloat(&duration);
		restore->ReadFloat(&starttime);
		restore->ReadFloat(&timeelapsed);
		return;
	}

	//read all the other data
	restore->ReadString(g_SongToTransition.SongName, sizeof(g_SongToTransition.SongName), 0);
	restore->ReadInt(&g_SongToTransition.pitch);
	restore->ReadFloat(&g_SongToTransition.Volume);
	restore->ReadFloat(&g_SongToTransition.SongDuration);
	restore->ReadFloat(&g_SongToTransition.SongStartTime);
	restore->ReadFloat(&g_SongToTransition.TimeElapsed);

	//tell the game to play the song when everything is loaded
	bShouldPlayOnceLoaded = true;
}

//song transition system auto system
class CSongTransitionSystem : public CAutoGameSystem
{
public:
	void LevelShutdownPostEntity()
	{
		...
		g_SongToTransition.TimeElapsed = g_SongToTransition.TimeElapsed - (gpGlobals->curtime - g_SongToTransition.SongStartTime);
	...
	void LevelInitPostEntity()
	{
		...
		if (gpGlobals->eLoadType == MapLoadType_t::MapLoad_LoadGame && bShouldPlayOnceLoaded)
		{
			//play the song
			Amod_PlaySong(g_SongToTransition.SongName, g_SongToTransition.Volume, g_SongToTransition.pitch, g_SongToTransition.TimeElapsed + g_SongToTransition.SongStartTime);
			bShouldPlayOnceLoaded = false;
			return;
		}

		//check map load type. If not 'level transition' then mark the song as
		//not in use.
		if (gpGlobals->eLoadType != MapLoadType_t::MapLoad_Transition)
		{
			//reset everything
			g_SongToTransition.bInUse = false;
			g_SongToTransition.pitch = 100;
		...
		//get the start time
		g_SongToTransition.SongStartTime = gpGlobals->curtime;

		//play the sound
		Amod_PlaySong(g_SongToTransition.SongName, g_SongToTransition.Volume, g_SongToTransition.pitch, g_SongToTransition.TimeElapsed + g_SongToTransition.SongStartTime);
	}
};
static CSongTransitionSystem g_SongTransitionSystem;

//marks the song as 'not in use'
CON_COMMAND_F(amod_remove_previous_song, "", FCVAR_HIDDEN)
{
	g_SongToTransition.bInUse = false;
}

ConVar amod_songs_transition_through_levels("amod_songs_transition_through_levels", "1");
...
void CAmbientGeneric::SendSound( SoundFlags_t flags)
{
	...
	if ( pSoundSource )
	{
		...
		else
		{
			...
			char szSound[1028];
			Q_strcpy(szSound, szSoundFile);

			//is this sound a song? must be from the music dir, not a background map and not be the elevator music
			bool IsSong = (Q_strstr(szSound, "music/") == szSound || Q_strstr(szSound, "music\\") == szSound) && gpGlobals->eLoadType != MapLoadType_t::MapLoad_Background && !Q_strstr(szSound, "elevator_music.wav");
			bool IsFallSong = !Q_stricmp(szSound, "fall_music.mp3") || !Q_stricmp(szSound, "fall.wav");
			
			//check amod_music_disable.GetBool()
			if ((IsFallSong || IsSong) && gpGlobals->eLoadType != MapLoadType_t::MapLoad_Background && amod_music_disable.GetBool())
				return;

			//HACK: check for song name with .mp3 extention and change them to be .wav
			if (IsSong && Q_GetFileExtension(szSound) && !Q_stricmp(Q_GetFileExtension(szSound), "mp3"))
			{
				Q_SetExtension(szSound, "wav", sizeof(szSound));

				//precache the new sound
				PrecacheSound(szSound);
			}

			if ((IsFallSong || IsSong) && flags != SND_STOP)
			{
				flags = SND_SHOULDPAUSE;
			}

			if (IsSong && amod_songs_transition_through_levels.GetBool())
			{
				//is the song still playing. if not then mark the song as 'not in use'
				if (g_SongToTransition.bInUse && fabs(g_SongToTransition.TimeElapsed - (gpGlobals->curtime - g_SongToTransition.SongStartTime)) >= g_SongToTransition.SongDuration)
					g_SongToTransition.bInUse = false;

				if (g_SongToTransition.bInUse)
					return;

				//set the song transition list item
				Q_strcpy(g_SongToTransition.SongName, szSound);
				g_SongToTransition.SongStartTime = gpGlobals->curtime;
				g_SongToTransition.Volume = m_iHealth;
				g_SongToTransition.SongDuration = enginesound->GetSoundDuration(szSound) * ((float)m_dpv.pitch/100);
				g_SongToTransition.pitch = m_dpv.pitch;
				g_SongToTransition.TimeElapsed = 0.0f;
				g_SongToTransition.bInUse = true;
			}

			if (!IsSong)
			{
				UTIL_EmitAmbientSound(pSoundSource->GetSoundSourceIndex(), pSoundSource->GetAbsOrigin(), szSound,
			...
			else
			{
				...
				Amod_PlaySong(szSound, m_iHealth, m_dpv.pitch, 0.0f, true);
			}
		}
	}	
	else

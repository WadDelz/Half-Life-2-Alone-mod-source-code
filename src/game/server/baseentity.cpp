...
void CBaseEntity::TakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if (inputInfo.GetAttacker() == pPlayer && IsNPC() && prevhealth != GetHealth())
	{
		CSingleUserRecipientFilter filter(pPlayer);

		//get damage position
		Vector pos = inputInfo.GetDamagePosition();

		//if too far away from this then just use eye position - 25
		...
		UserMessageBegin(filter, "ShowHitmarker");
			WRITE_BYTE(1);
			WRITE_BYTE(IsAlive());
			WRITE_BYTE(Clamp<int>(inputInfo.GetDamage(), 0, GetMaxHealth()));
			WRITE_VEC3COORD(pos);
		MessageEnd();
	}
}

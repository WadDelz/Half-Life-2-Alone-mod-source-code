...
//	DEFINE_CUSTOM_FIELD( m_Activity, ActivityDataOps() ),
	...
	DEFINE_FIELD(f_dobreath, FIELD_TIME),
	DEFINE_FIELD(f_doweatherinfo, FIELD_TIME),
...
extern ConVar amod_do_breathing;
extern ConVar amod_weather_info;
...
WeatherInfo_t GetWeatherMessageInfo(CBasePlayer* pPlayer)
{
	...
	//get the current weather
	{
		...
		//check for bonus snowey maps
		for (int i = 0; i < sizeof(g_pBonusMaps) / sizeof(g_pBonusMaps[0]); i++)
		{
			if (!Q_strcmp(mapname, CFmtStr("bonus/%s", g_pBonusMaps[i])))
			{
				info.m_CurrentWeather = "Snowey";
				WeatherClear = false;
				break;
			}
		}
		...
		//check for rain
		if (amod_rain_enable.GetBool() && WeatherClear)
		{
			//is rainy
			if (amod_rain_type.GetInt() == 1 || (amod_rain_type.GetInt() == 2 && pPlayer->m_bInRain))
			{
				info.m_CurrentWeather = "Rainy";
				WeatherClear = false;
			}
		}

		//check for episode 1 map
		if (StringHasPrefix(mapname, "ep1_") && WeatherClear)
		{
			info.m_CurrentWeather = "Ashy";
			WeatherClear = false;
		}
		...
		if (WeatherClear)
		{
			info.m_CurrentWeather = "Clear";
		}
	}

	//get the temeperature
	{
		info.m_Temperature = random->RandomInt(10.0, 14.0);

		//station maps
		if (StringHasPrefix(mapname, "d1_trainstation_"))
			info.m_Temperature = random->RandomFloat(10.0, 14.0);
		
		//canals maps
		else if (StringHasPrefix(mapname, "d1_canals_"))
			info.m_Temperature = random->RandomFloat(8.0, 12.0);

		//eli's lab
		else if (StringHasPrefix(mapname, "d1_eli_"))
			info.m_Temperature = random->RandomFloat(11.0, 17.0);
		
		//town
		else if (StringHasPrefix(mapname, "d1_town"))
			info.m_Temperature = random->RandomFloat(7.0, 14.0);
		
		//coast
		else if (StringHasPrefix(mapname, "d2_coast"))
			info.m_Temperature = random->RandomFloat(5.0, 10.0);
		
		//prison
		else if (StringHasPrefix(mapname, "d2_prison"))
			info.m_Temperature = random->RandomFloat(9.0, 15.0);
		
		//city
		else if (StringHasPrefix(mapname, "d3_c17_"))
			info.m_Temperature = random->RandomFloat(8.0, 15.0);
		
		//citadel
		else if (StringHasPrefix(mapname, "d3_citadel_") || StringHasPrefix(mapname, "d3_breen_"))
			info.m_Temperature = random->RandomFloat(3.0, 7.0);

		//ep1 citadel
		else if (StringHasPrefix(mapname, "ep1_citadel_"))
			info.m_Temperature = random->RandomFloat(5.0, 9.0);

		//ep1
		else if (StringHasPrefix(mapname, "ep1_"))
			info.m_Temperature = random->RandomFloat(12.0, 19.0);

		//ep1
		else if (StringHasPrefix(mapname, "ep2_"))
			info.m_Temperature = random->RandomFloat(5.0, 13.0);
		...
		for (int i = 0; i < sizeof(g_pBonusMaps) / sizeof(g_pBonusMaps[0]); i++)
		{
			if (!Q_strcmp(mapname, CFmtStr("bonus/%s", g_pBonusMaps[i])))
			{
				info.m_Temperature = random->RandomInt(-19.0, -14.0);
				break;
			}
		}
	}
...
void CBasePlayer::PostThink()
{
	...
	//show weather info
	if (gpGlobals->curtime >= f_doweatherinfo)
	{
		if (amod_weather_info.GetBool())
		{
			//get the weather info
			WeatherInfo_t info = GetWeatherMessageInfo(this);

			//check the forcast
			const char* message = nullptr;
			if (amod_rain_enable.GetBool() && amod_rain_type.GetInt() == 2 && !m_bInRain)
			{
				int minutes = (int)((m_fNextRainTime - gpGlobals->curtime) / 60); // integer division gives full minutes
				int seconds = (int)(m_fNextRainTime - gpGlobals->curtime) % 60; // remainder after dividing by 60 gives seconds

				message = CFmtStr1024("Temperature is Currently %.1fc\nWeather Is %s\nThere is a rainstorm forecast to be here in roughly %d:%.2d minutes%s%s", info.m_Temperature, info.m_CurrentWeather, minutes, seconds, amod_rain_thunder.GetBool() ? "\n" : "", amod_rain_thunder.GetBool() ? "Listen out for thunder!" : "");
			}
			else
			{
				message = CFmtStr1024("Temperature is Currently %.1fc\nWeather Is %s%s%s", info.m_Temperature, info.m_CurrentWeather, amod_rain_thunder.GetBool() ? "\n" : "", amod_rain_thunder.GetBool() ? "Listen out for thunder!" : "");
			}

			UTIL_HudHintText(this, message);
		}

		f_doweatherinfo = gpGlobals->curtime + random->RandomInt(60, 120);
	}
...
//rain
extern ConVar amod_rain_splash_particle_name;

void CBasePlayer::Precache( void )
{
	BaseClass::Precache();

	//precache rain particle
	PrecacheParticleSystem(amod_rain_splash_particle_name.GetString());
	PrecacheParticleSystem("fog_breath");

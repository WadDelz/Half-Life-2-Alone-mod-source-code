...
bool COverlayPageList::ShouldOverlayDraw(C_BaseHLPlayer *pPlayer, OverlayDrawType_e type)
{
	...
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenUnderWater) && pPlayer->GetWaterLevel() < WL_Eyes)
		return true;

	//check for crouching
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenCrouching) && (pPlayer->GetFlags() & FL_DUCKING))
		return true;

	//check for on ground
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenOnGround) && (pPlayer->GetFlags() & FL_ONGROUND))
		return true;

	//check for holding object
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenHoldingObject) && pPlayer->GetUseEntity())
		return true;

	//check for zooming
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenUsingSuitZoom) && pPlayer->IsZoomed())
		return true;
	
	//check for in air
	if (((int)type & (int)OverlayDrawType_e::Draw_WhenInAir) && !(pPlayer->GetFlags() & FL_ONGROUND))
		return true;
	...
	: BaseClass(parent, name)
{
	...
	{
		m_TypeComboBox->GetMenu()->AddCheckableMenuItem(g_OverlayDrawStrings[i], new KeyValues("SetText", "text", g_OverlayDrawStrings[i]), this, nullptr);
		m_TypeComboBox->GetMenu()->GetMenuItem(i)->SetCommand(CFmtStr(OVERLAY_PAGE_OVERLAY_TYPE_PREFIX "%d", i));
...
void CEffectsPanelOverlayPage::OnCommand(const char* pszCommand)
{
	...
	if (!Q_strcmp(pszCommand, ADD_OVERLAY_BUTTON_COMMAWND))
	{
		...
		if (!m_OverlayList->AddOverlay(buf, m_RedSlider->GetValue(), m_GreenSlider->GetValue(), m_BlueSlider->GetValue(), m_AlphaSlider->GetValue(), (OverlayDrawType_e)drawtype))
		{
			//play an error sound
			vgui::surface()->PlaySound("resource/warning.wav");
	...
	else if (!Q_strcmp(pszCommand, CHANGE_OVERLAY_BUTTON_COMMAWND))
	{
		...
		if (!m_OverlayList->ChangeSelectedOverlay(buf, m_RedSlider->GetValue(), m_GreenSlider->GetValue(), m_BlueSlider->GetValue(), m_AlphaSlider->GetValue(), (OverlayDrawType_e)drawtype))
		{
			//play an error sound
			vgui::surface()->PlaySound("resource/warning.wav");

...
#pragma region Bloom
	...
	CMapPropertiesPanelSlider* m_BloomSlider;
	Label* m_BloomText;
...
#pragma endregion
	...
	//are we closing this panel with the modal prompt
	bool m_bClosingFromModal;
};
...
CMapPropertiesPanel::CMapPropertiesPanel(Panel* parent) : BaseClass(parent, "MapPropertiesPanel")
{
	...
	//create the fog sliders
	{
		...
		m_FogDensitySlider->SetRange(-1, 100);
		m_SkyboxFogDensitySlider->SetRange(-1, 100);
	}
	...
	//post processing
	{
		...
		m_IntensitySlider->SetBounds(260, 370, 280, 22);
	...
	{
		//title
		Label* title = new Label(this, "CloudsTitle", "Cloud Colors:");
	...
	//create the divider for the bloom
	Divider* bloomdivider = new Divider(this, "bloomdivider");
	bloomdivider->SetBounds(245, 468, 400, 1);

	//bloom
	{
		//title
		Label* title = new Label(this, "BloomTitle", "Bloom Settings");
		title->SetBounds(245, 468, 305, 20);
		title->SetContentAlignment(Label::Alignment::a_center);
		...
		//bloom amount slider
		m_BloomSlider = new CMapPropertiesPanelSlider(this, "BloomSlider");
		m_BloomSlider->SetBounds(255, 510, 250, 20);
		m_BloomSlider->SetRange(1, 100);

		//bloom amount text
		m_BloomText = new Label(this, "BloomText", "0");
		m_BloomText->SetBounds(505, 510, 100, 20);

		//bloom scalar slider
		m_BloomScalarSlider = new CMapPropertiesPanelSlider(this, "BloomSlider");
		m_BloomScalarSlider->SetBounds(255, 530, 250, 20);
		m_BloomScalarSlider->SetRange(1, 10000);

		//bloom scalar amount text
		m_BloomScalarText = new Label(this, "BloomText", "0");
		m_BloomScalarText->SetBounds(505, 530, 100, 20);
	}

	//create the divider for the skybox + bloom
	...
	m_ApplyButton->SetBounds(20, 769, 510, 20);
}
...
void CMapPropertiesPanel::OnThink()
{
	//set the fog values
	{
		...
		//values can be -1
		{
			int start = m_FogStartSlider->GetValue();
			int end = m_FogEndSlider->GetValue();
			int skyboxstart = m_SkyboxFogStartSlider->GetValue();
			int skyboxend = m_SkyboxFogEndSlider->GetValue();
			...
			m_FogStartSlider->GetRange(mins, _);
			m_FogEndSlider->GetRange(mine, _);
			m_SkyboxFogStartSlider->GetRange(minss, _);
			m_SkyboxFogEndSlider->GetRange(mines, _);

			//set value
			fog_start->SetValue(start <= mins ? -1 : start);
			fog_end->SetValue(end <= mine ? -1 : end);
			fog_startskybox->SetValue(skyboxstart <= minss ? -1 : skyboxstart);
			fog_endskybox->SetValue(skyboxend <= mines ? -1 : skyboxend);
		}

		//values can be -1
		{
			int densityvalue = m_FogDensitySlider->GetValue();
			int skyboxdensityvalue = m_SkyboxFogDensitySlider->GetValue();

			fog_maxdensity->SetValue(densityvalue < 0 ? -1 : (float)densityvalue / 100);
			fog_maxdensityskybox->SetValue(skyboxdensityvalue < 0 ? -1 : (float)skyboxdensityvalue / 100);
		}
	...
	//bloom
	{
		...
		mat_bloomscale->SetValue(m_BloomSlider->GetValue());
		mat_bloom_scalefactor_scalar->SetValue((float)m_BloomScalarSlider->GetValue() / 100);

		//set text
		m_BloomText->SetText(CFmtStr("%d", mat_bloomscale->GetInt()));
		m_BloomScalarText->SetText(CFmtStr("%.3f", mat_bloom_scalefactor_scalar->GetFloat()));

		//set enaboled
		m_BloomSlider->SetEnabled(m_EnableBloomCheckButton->IsSelected());
...
void CMapPropertiesPanel::OnCommand(const char* pszCommand)
{
	...
	if (!Q_stricmp(pszCommand, COMMAND_APPLY_PAGE_SETTINGS_CONFIRM))
	{
		//reload the settings for the server. We could have changed some
...
{	
	...
	//FOG
	{
		...
		//get the other fog values (can be -1)
		{
			int fogstart = atoi(FindFogInfoFromArray(FogInfo, "fog_start", "500"));
			int fogend = atoi(FindFogInfoFromArray(FogInfo, "fog_end", "2000"));
			int fogstartskybox = atoi(FindFogInfoFromArray(FogInfo, "fog_startskybox", "10000"));
			int fogendskybox = atoi(FindFogInfoFromArray(FogInfo, "fog_endskybox", "35000"));
			...
			//set
			m_FogStartSlider->SetValue(fogstart == -1 ? mins : fogstart);
			m_FogEndSlider->SetValue(fogend == -1 ? mine : fogend);
			m_SkyboxFogStartSlider->SetValue(fogstartskybox == -1 ? minss : fogstartskybox);
			m_SkyboxFogEndSlider->SetValue(fogendskybox == -1 ? mines : fogendskybox);
		}

		//value can be -1
		{
			float density = atof(FindFogInfoFromArray(FogInfo, "fog_maxdensity", "1.0"));
			float skyboxdensity = atof(FindFogInfoFromArray(FogInfo, "fog_maxdensityskybox", "1.0"));

			m_FogDensitySlider->SetValue(density < 0 ? -1 : density * 100);
			m_SkyboxFogDensitySlider->SetValue(skyboxdensity < 0 ? -1 : skyboxdensity * 100);	
		}
	}
	...
	//BLOOM
	{
		...
		m_BloomSlider->SetValue(atoi(StringFromMapTimeStringTableIndex(IsNightPage ? info.NightInfo.BloomScale : info.DayInfo.BloomScale)));
		m_BloomScalarSlider->SetValue(atof(StringFromMapTimeStringTableIndex(IsNightPage ? info.NightInfo.BloomScalarFactor : info.DayInfo.BloomScalarFactor)) * 100);
	}
...
void CMapPropertiesPanel::GetData(MapTimeInfo_t& info)
{
	...
	do
	{
		...
		//set the info
		MapTimeInfo_t::FogInfo_t fog_override{ StringToMapTimeStringTableIndex("fog_override"),StringToMapTimeStringTableIndex(CFmtStr("%d", m_OverrideFog->IsSelected())) };
		...
		MapTimeInfo_t::FogInfo_t fog_startskybox{ StringToMapTimeStringTableIndex("fog_startskybox"),StringToMapTimeStringTableIndex(CFmtStr("%d", m_SkyboxFogStartSlider->GetValue())) };
		MapTimeInfo_t::FogInfo_t fog_endskybox{ StringToMapTimeStringTableIndex("fog_endskybox"),StringToMapTimeStringTableIndex(CFmtStr("%d", m_SkyboxFogEndSlider->GetValue())) };
		...
		FogInfo.AddToTail(fog_override);
	...
	do
	{
		...
		if (m_bNightTimeMode)
		{
			...
			info.NightInfo.BloomScale = StringToMapTimeStringTableIndex(CFmtStr("%d", m_BloomSlider->GetValue()));
			info.NightInfo.BloomScalarFactor = StringToMapTimeStringTableIndex(CFmtStr("%.3f", (float)m_BloomScalarSlider->GetValue() / 100));
		}
		else
		{
			info.DayInfo.BloomEnabled = StringToMapTimeStringTableIndex(CFmtStr("%d", m_EnableBloomCheckButton->IsSelected()));
			info.DayInfo.BloomScale = StringToMapTimeStringTableIndex(CFmtStr("%d", m_BloomSlider->GetValue()));
			info.DayInfo.BloomScalarFactor = StringToMapTimeStringTableIndex(CFmtStr("%.3f", (float)m_BloomScalarSlider->GetValue()/100));
		}
...
static MapTimeInfo_t::DayInfo_t gs_CopiedState[2];		//the night info has the same data the DayInfo_t struct has.
...
CMapPropertiesEditorPageBase::CMapPropertiesEditorPageBase(Panel* parent, const char* name, bool IsNightPage) : BaseClass(parent, name)
{
	...
	{
		memset(&gs_CopiedState[0], 0, sizeof(MapTimeInfo_t::DayInfo_t));		//sizeof(MapTimeInfo_t::DayInfo_t) looks cooler in editor
		memset(&gs_CopiedState[1], 0, sizeof(MapTimeInfo_t::DayInfo_t));		//sizeof(MapTimeInfo_t::DayInfo_t) looks cooler in editor
	}
...
void CMapPropertiesEditorPageBase::OnCommand(const char* cmd)
{
	...
	else if (Q_strstr(cmd, COPY_MAP_STATE_PREFIX))
	{
		...
		{
			//clear current data
			gs_CopiedState[0].FogInfo.RemoveAll();

			//set new state
			gs_CopiedState[0].CloudsColor = info.NightInfo.CloudsColor;
			gs_CopiedState[0].DefaultDaySky = info.NightInfo.DefaultNightSky;
			gs_CopiedState[0].FilterIntensity = info.NightInfo.FilterIntensity;
			gs_CopiedState[0].FilterName = info.NightInfo.FilterName;
			gs_CopiedState[0].FogEnabled = info.NightInfo.FogEnabled;
			gs_CopiedState[0].BloomEnabled = info.NightInfo.BloomEnabled;
			gs_CopiedState[0].BloomScale = info.NightInfo.BloomScale;
			gs_CopiedState[0].BloomScalarFactor = info.NightInfo.BloomScalarFactor;

			//copy the fog states
			for (int i = 0; i < info.NightInfo.FogInfo.Count(); i++)
			{
				MapTimeInfo_t::FogInfo_t& finfo = gs_CopiedState[0].FogInfo[gs_CopiedState[0].FogInfo.AddToTail()];
				finfo.convar = info.NightInfo.FogInfo[i].convar;
				finfo.value = info.NightInfo.FogInfo[i].value;
			}
		}
		else
		{
			//clear current data
			gs_CopiedState[1].FogInfo.RemoveAll();
			gs_CopiedState[1].SunInfo.RemoveAll();

			//set new state
			gs_CopiedState[1].CloudsColor = info.DayInfo.CloudsColor;
			gs_CopiedState[1].DefaultDaySky = info.DayInfo.DefaultDaySky;
			gs_CopiedState[1].FilterIntensity = info.DayInfo.FilterIntensity;
			gs_CopiedState[1].FilterName = info.DayInfo.FilterName;
			gs_CopiedState[1].FogEnabled = info.DayInfo.FogEnabled;
			gs_CopiedState[1].SunInfoEnabled = info.DayInfo.SunInfoEnabled;
			gs_CopiedState[1].BloomEnabled = info.DayInfo.BloomEnabled;
			gs_CopiedState[1].BloomScale = info.DayInfo.BloomScale;
			gs_CopiedState[1].BloomScalarFactor = info.DayInfo.BloomScalarFactor;

			//copy the fog states
			for (int i = 0; i < info.DayInfo.FogInfo.Count(); i++)
			{
				MapTimeInfo_t::FogInfo_t& finfo = gs_CopiedState[1].FogInfo[gs_CopiedState[1].FogInfo.AddToTail()];
				finfo.convar = info.DayInfo.FogInfo[i].convar;
				finfo.value = info.DayInfo.FogInfo[i].value;
			}

			//copy the sun states
			for (int i = 0; i < info.DayInfo.SunInfo.Count(); i++)
			{
				MapTimeInfo_t::DayInfo_t::SunInfo_t& finfo = gs_CopiedState[1].SunInfo[gs_CopiedState[1].SunInfo.AddToTail()];
				finfo.key = info.DayInfo.SunInfo[i].key;
				finfo.value = info.DayInfo.SunInfo[i].value;
			}
		}
	}
	...
	else if (Q_strstr(cmd, PASTE_MAP_STATE_PREFIX))
	{
		...
		if (m_bIsNightPage)
		{
			...
			//set new state
			info.NightInfo.CloudsColor = gs_CopiedState[0].CloudsColor;
			info.NightInfo.DefaultNightSky = gs_CopiedState[0].DefaultDaySky;
			info.NightInfo.FilterIntensity = gs_CopiedState[0].FilterIntensity;
			info.NightInfo.FilterName = gs_CopiedState[0].FilterName;
			info.NightInfo.FogEnabled = gs_CopiedState[0].FogEnabled;
			info.NightInfo.BloomEnabled = gs_CopiedState[0].BloomEnabled;
			info.NightInfo.BloomScale = gs_CopiedState[0].BloomScale;
			info.NightInfo.BloomScalarFactor = gs_CopiedState[0].BloomScalarFactor;

			//paste the fog states
			for (int i = 0; i < gs_CopiedState[0].FogInfo.Count(); i++)
			{
				MapTimeInfo_t::FogInfo_t& finfo = info.NightInfo.FogInfo[info.NightInfo.FogInfo.AddToTail()];
				finfo.convar = gs_CopiedState[0].FogInfo[i].convar;
				finfo.value = gs_CopiedState[0].FogInfo[i].value;
			}
		}
		else
		{
			...
			//set new state
			info.DayInfo.CloudsColor = gs_CopiedState[1].CloudsColor;
			info.DayInfo.DefaultDaySky = gs_CopiedState[1].DefaultDaySky;
			info.DayInfo.FilterIntensity = gs_CopiedState[1].FilterIntensity;
			info.DayInfo.FilterName = gs_CopiedState[1].FilterName;
			info.DayInfo.FogEnabled = gs_CopiedState[1].FogEnabled;
			info.DayInfo.SunInfoEnabled = gs_CopiedState[1].SunInfoEnabled;
			info.DayInfo.BloomEnabled = gs_CopiedState[1].BloomEnabled;
			info.DayInfo.BloomScale = gs_CopiedState[1].BloomScale;
			info.DayInfo.BloomScalarFactor = gs_CopiedState[1].BloomScalarFactor;

			//paste the fog states
			for (int i = 0; i < gs_CopiedState[1].FogInfo.Count(); i++)
			{
				MapTimeInfo_t::FogInfo_t& finfo = info.DayInfo.FogInfo[info.DayInfo.FogInfo.AddToTail()];
				finfo.convar = gs_CopiedState[1].FogInfo[i].convar;
				finfo.value = gs_CopiedState[1].FogInfo[i].value;
			}

			//paste the sun states
			for (int i = 0; i < gs_CopiedState[1].SunInfo.Count(); i++)
			{
				MapTimeInfo_t::DayInfo_t::SunInfo_t& sinfo = info.DayInfo.SunInfo[info.DayInfo.SunInfo.AddToTail()];
				sinfo.key = gs_CopiedState[1].SunInfo[i].key;
				sinfo.value = gs_CopiedState[1].SunInfo[i].value;
			}
		}

...
private:
	...
	FileOpenDialog* m_FOLoad = nullptr;
	FileOpenDialog* m_FSave = nullptr;
	bool bWasLoad;
};					
...
void CEffectsPanel::Init()
{
	...
	Button* ResetEffects = new Button(this, "", "Reset Effects");
	ResetEffects->SetCommand("Reset");
	ResetEffects->SetBounds(10, 455, 220, 25);

	Label* Sov = new Label(this, "", "This Was Mainly Made For People To\nCreate Cinematic Style Videos");
	Sov->SetBounds(10, 415, 250, 35);
}
...
void CEffectsPanel::OnCommand(const char* pcCommand)
{
	...
	else if (!Q_strcmp(pcCommand, "Load"))
	{
		...
		if (!m_FOLoad)
		{
			...
			m_FOLoad->SetStartDirectory(buf);
		}
	...
	else if (!Q_strcmp(pcCommand, "Save"))
	{
		...
		if (!m_FSave)
		{
			...
			char buf[512];
			g_pFullFileSystem->RelativePathToFullPath("scripts", "MOD", buf, sizeof(buf));
			m_FSave->SetStartDirectory(buf);
		}
		m_FSave->DoModal(false);
		m_FSave->Activate();
	}
	else if (!Q_strcmp(pcCommand, "Reset"))
	{
		m_cbBAWhite->SetSelected(false);
		m_cbBCam->SetSelected(false);
		m_cbBinoculars->SetSelected(false);
		m_cbCBlur->SetSelected(false);
		m_cbLenseDirt->SetSelected(false);
		m_cbCPhobia->SetSelected(false);
		m_sCPhobia->SetValue(false);
		m_sCFov->SetValue(false);
		m_cbSquare->SetSelected(false);
		m_sSquare->SetValue(false);
		m_sSquareHeight->SetValue(false);

		m_CBFilter1->SetSelected(false);
		m_CBFilter2->SetSelected(false);
		m_CBFilter3->SetSelected(false);
		m_CBFilter4->SetSelected(false);
		m_CBFilter5->SetSelected(false);
		m_CBFilter6->SetSelected(false);
		m_CBFilter7->SetSelected(false);
		m_CBFilter8->SetSelected(false);

		m_cbVFov->SetSelected(false);
		m_sViewmodelFov->SetValue(54);

		v_viewmodel_fov.SetValue(m_sViewmodelFov->GetValue());
		fov_desired.SetValue(90);

		m_cbCCamera->SetSelected(false);
		m_cbCCameraFix->SetSelected(false);
		m_cbDDVModel->SetSelected(false);
		m_sCCamAngles->SetValue(5);
		m_sCCamOrigin->SetValue(5);
...
void CEffectsPanel::OnFileSelected(const char* pszFileName)
{
	...
	else
	{
		...
		char buf[1028];
		Q_strcpy(buf, pszFileName);

		if (Q_strcmp(Q_GetFileExtension(pszFileName), "amf"))
			Q_SetExtension(buf, "amf", sizeof(buf));

		kv->SaveToFile(g_pFullFileSystem, buf);
	}
}
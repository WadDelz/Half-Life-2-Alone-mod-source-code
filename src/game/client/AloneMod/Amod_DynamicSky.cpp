//not really a "dynamic sky" just a thing that makes it so you can change the sky in real time instead of having to reload the game/save/map etc..
#include "cbase.h"
#include "AloneMod/Amod_DynamicSky.h"
...
//-----------------------------------------------------------------------------
// Purpose: change callback for sv_skyname convar
//-----------------------------------------------------------------------------
void SvSkyboxChangeCallback(IConVar*, const char*, float)
{
	//i have no idea if this will cause memory leaks but we will soon find out (:
	R_UnloadSkys();
	R_LoadSkys();
}
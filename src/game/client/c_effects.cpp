...
ConVar amod_rain_splash_particle_name("amod_rain_splash_particle_name", "water_splash_01", FCVAR_REPLICATED);
ConVar amod_rain_splashes("amod_rain_splashes", "1");
...
// Globals
//-----------------------------------------------------------------------------
void RainDensityChangeCallback(IConVar*, const char*, float);

ConVar CClient_Precipitation::s_raindensity( "r_raindensity","0.001", FCVAR_NONE, "", RainDensityChangeCallback);
...
//changes the rain density in real time
void RainDensityChangeCallback(IConVar*, const char*, float)
{
	...
	{
		if (dynamic_cast<CClient_Precipitation*>(pEntity))
		{
			pEntity->OnDataChanged(DataUpdateType_t::DATA_UPDATE_DATATABLE_CHANGED);
		}
	}

	//restart the rain sounds
	engine->ClientCmd_Unrestricted("amod_soundscape_startrain");

	//get the current rain farz
	engine->ClientCmd_Unrestricted("exec \"rain/background_main");
	engine->ClientCmd_Unrestricted(CFmtStr("exec \"rain/%s", szMapName));
	engine->ClientCmd_Unrestricted("rain_density_check");	//do te rain density check after the file's get executed
}

CON_COMMAND_F(rain_density_check, "", FCVAR_HIDDEN)
{
	//set the rain radius based on the density
	if (CClient_Precipitation::s_raindensity.GetFloat() < 0.001)
		engine->ClientCmd_Unrestricted(CFmtStr("r_rainradius %d", (int)(r_RainRadius.GetInt() * 1.3f)));
...
inline bool CClient_Precipitation::SimulateRain( CPrecipitationParticle* pParticle, float dt )
{
	...
	for ( int i = 0 ; i < 2 ; i++ )
		...
		if ( !IsInAir( pParticle->m_Pos ) )
		{
			...
			{
				if ( RandomInt( 0, 450 ) < r_RainSplashPercentage.GetInt() && amod_rain_splashes.GetBool())
				{
					trace_t trace;
					UTIL_TraceLine(vOldPos, pParticle->m_Pos, MASK_SOLID | MASK_WATER, NULL, COLLISION_GROUP_NONE, &trace);
...
void CClient_Precipitation::CreateWaterSplashes()
{
	for ( int i=0; i < m_Splashes.Count(); i++ )
	{
		...
		if (!pPlayer || pPlayer->GetAbsOrigin().DistTo(vSplash) > (r_RainRadius.GetInt() / 2))
			continue;
		...
		if ( CurrentViewForward().Dot( vSplash - CurrentViewOrigin() ) > 1 )
		{
			...
			DispatchParticleEffect(amod_rain_splash_particle_name.GetString(), vSplash, vec3_angle);
		}
	}
	m_Splashes.Purge();
}

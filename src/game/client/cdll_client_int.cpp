...
void HideMainMenuLogoRecursive(vgui::VPANEL rootpanel)
{
	...
	for (int i = 0; i < vgui::ipanel()->GetChildCount(rootpanel); i++)
	{
		...
		//if the name is GameMenuButton or GameMenuButton2 then remove then. these are the
		//names of the text logos
...
//-----------------------------------------------------------------------------
// Purpose: initalize all the background holders and check to see if we are
...
void ModBase_InitBackground()
{
	...
	//try to open modbase/config/background_menu.txt
	if (!_BackgroundConfig->LoadFromFile(g_pFullFileSystem, "modbase/config/background_menu.txt", "MOD"))
	{
		BikBackgroundDebugMsg("Background BIK Video: Failed to load 'modbase/config/background_menu.txt!'\n");
	...
	//check the convar is valid. if not use the default convar
	...
	FOR_EACH_TRUE_SUBKEY(_BackgroundConfig, data)
	{
		...
		//the min value isnt used for ComparePreviousMap. For that it uses 'IsDefault'
	...
	//check if the texture name isnt nullptr. if it is then make the mod not use the texture logo
...
void CHLClient::PostInit()
{
	...
	//only call this if bik videos are enabled and gamepadui isnt enabled (do the gamepad ui thing after)
	if (g_PModBase_BikBackground_bUse && !g_PModBase_BIsGamepadUI)
	{
		SwapDisconnectCommand();

		//if the +map parameter is in use. dont bother disconnecting
		if (CommandLine()->CheckParm("+map") == nullptr)
...
void CHLClient::HudUpdate( bool bActive )
{
	...
	//alright so when the gamepadui is active and we are running 'debug' mode then it wont actually
	...
	//to see if we are loading into a level or if we arn't loading into a level (are disconnected). If the gamepadui 
	//is active and we arnt loading into a background then usually the background bik video wont play, 
	//This is because for the gamepadui the bik loading functionality is in the LevelInitPostEntity() function.
	//And that doesnt get called if we arnt connected/loading into a server.
...
void CHLClient::LevelInitPostEntity( )
{
	...
	//called on startup for the gamepadui. The reason i do this is because when loading into a background map is because if this gets
	//called on PostInit() for portal 1 (or any source engine game that has the spinning loading disc in the top right corner) then it will
	//cause the game to go into an infinate loop. The only way to fix it is by doing this because the spinning wheel functionality is in the
	//GameUI dll
...
void OnRenderStart()
{
	...
	//alone mod: update effects panel lights
	Amod_EffectsPanelUpdateLighting();

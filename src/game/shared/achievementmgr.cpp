...
CON_COMMAND_F(_do_achievement_notification, "", FCVAR_HIDDEN)
{
	...
	int goal = atoi(args.Arg(2));

	//get message
	const char* message = CFmtStr1024("Achievement: '%ls' Completed!", achievement_name);
	if (progress < goal)
		message = CFmtStr1024("%ls (%d / %d) Completed!", achievement_name, progress, goal);

	//convert to wchar
	wchar_t buf[1028];
	Q_strtowcs(message, -1, buf, sizeof(buf));

	//show notification
	CAchievementNotificationPanel* panel = GET_HUDELEMENT(CAchievementNotificationPanel);
	if (panel)
		panel->AddNotification(args.Arg(0), progress >= goal ? L"Achievement Earnt" : L"Achievement Progress", buf);
}
#endif
...
#ifndef CLIENT_DLL
	static ConCommand* cmd = cvar->FindCommand("_do_achievement_notification");
	if (cmd)
	{
		CCommand args;
		args.Tokenize(CFmtStr1024("%s %d %d", pAchievement->GetName(), pAchievement->GetCount(), pAchievement->GetGoal()));
		cmd->Dispatch(args);
	}
#endif
...
#if /*defined(_DEBUG) || defined(STAGING_ONLY) || DEBUG_ACHIEVEMENTS_IN_RELEASE*/1

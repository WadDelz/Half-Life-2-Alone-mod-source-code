...
static ConVar amod_achievements_show_increment_hidden("amod_achievements_show_increment_hidden", "0");
...
#ifndef CLIENT_DLL
	if (prevcount != m_iCount && m_iCount != m_iGoal)
	{
		//see if hidden
		if (!m_bHideUntilAchieved)
		{
			static ConCommand* cmd = cvar->FindCommand("_do_achievement_notification");
			if (cmd)
			{
				CCommand args;
				args.Tokenize(CFmtStr1024("%s %d %d", GetName(), m_iCount, m_iGoal));
				cmd->Dispatch(args);
			}
		}
	}
